...
function Components(props) {
  ...
  return (
    <div className={styles.articles_item}>
      <div className={styles.articles_item_poster} style={{ backgroundImage: `url(${props.item.attributes.poster})` }}>
        {!props.item.attributes.poster && <FileImageOutlined className={styles.articles_item_poster_placeholder} />}
      ...
      <a className={styles.articles_item_content} target="_blank" href={`/_user/feature/articles/${props.item.id}`}>
        <p className={styles.articles_item_title}>{props.item.attributes.title}</p>
        <div className={styles.articles_item_bottom}>
          <p className={styles.articles_item_author}>{props.item.attributes.author.attributes.nickname}</p>
          <p className={styles.articles_item_updateAt}>{dayjs(props.item.attributes.updatedAt).format('YYYY/MM/DD')}更新</p>
        </div>
      </a>
      <Modal
        // title="上传封面"
